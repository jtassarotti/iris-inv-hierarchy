...
Definition twptp_pre (twptp : list (expr Λ) → iProp Σ)
  ...
  (∀ t2 σ1 ns κ κs σ2 nt, ⌜step (t1,σ1) κ (t2,σ2)⌝ -∗
    state_interp σ1 ns κs nt -∗ |NC={⊤}=> ∃ nt', ⌜κ = []⌝ ∗ state_interp σ2 (S ns) κs nt' ∗ twptp t2)%I.
...
Proof.
  ...
  iModIntro. iExists (length efs + nt). simpl. iFrame "Hσ".
...
Lemma twptp_total σ ns nt t :
  state_interp σ ns [] nt -∗ twptp t -∗ |NC={⊤}=> ▷ ⌜sn erased_step (t, σ)⌝.
Proof.
...
Theorem twp_total' Σ Λ `{!invPreG Σ} `{!crashPreG Σ} s e σ Φ (num_laters_per_step : nat → nat):
...
Proof.
  ...
  iApply (@twptp_total _ _ (IrisG _ _ Hinv Hcrash stateI fork_post _ _)
                       _ 0 with "Hσ").
  by iApply (@twp_twptp _ _ (IrisG _ _ Hinv Hcrash _ fork_post _ _)).
Qed.
...
Theorem twp_total Σ Λ `{!invPreG Σ} `{!crashPreG Σ} s e σ Φ :
  (∀ `{Hinv : !invG Σ} `{Hcrash: !crashG Σ},
     ⊢ |NC={⊤}=> ∃
         ...
         (fork_post : val Λ → iProp Σ),
       let _ : irisG Λ Σ :=
           IrisG _ _ Hinv Hcrash (λ σ _, stateI σ) fork_post (λ _, 0)
...
Proof.
  ...
  iApply (@twptp_total _ _ (IrisG _ _ Hinv Hcrash (λ σ _, stateI σ) fork_post _ _)
                       _ 0 with "Hσ").
  by iApply (@twp_twptp _ _ (IrisG _ _ Hinv Hcrash _ fork_post _ _)).
Qed.
